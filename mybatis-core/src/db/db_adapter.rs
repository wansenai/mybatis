                ...
                let p = build
                    .connect_with(
                        driver
                            .mysql
                            .clone()
                            .ok_or_else(|| Error::from("[mybatis-core] conn is none!"))?,
                    )
                    .await?;
        ...
        match self {
            ...
            DBPoolConn::Mysql(conn, decoder) => {
                ...
                let data = async_stream
                    ...
                    .as_array()
                    .ok_or_else(|| Error::from("[mybatis-core] try_to_json is not array!"))?
                    .to_owned();
...
impl<'a> DBTx<'a> {
    ...
    pub fn get_conn_mut(&mut self) -> crate::Result<&mut DBPoolConn<'a>> {
        self.conn
            .as_mut()
            .ok_or_else(|| Error::from("[mybatis-core] DBTx conn is none!"))
    }
