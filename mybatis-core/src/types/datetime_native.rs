        ...
        S: Serializer,
    {
        ...
        if type_name::<S::Error>().eq("rbson::ser::error::Error") {
            return serializer.serialize_str(&format!(
                "DateTimeNative({})",
                FormatWrapped { inner: &self }.to_string()
            ));
        } else {
        ...
        match Bson::deserialize(deserializer)? {
            ...
            Bson::String(s) => {
                ...
                if s.starts_with("DateTimeNative(") && s.ends_with(")") {
                    ...
                    return Ok(Self {
                        inner: chrono::NaiveDateTime::from_str(inner_data)
                            .or_else(|e| Err(D::Error::custom(e.to_string())))?,
                    });
                } else {
                    return Ok(Self {
                        inner: chrono::NaiveDateTime::from_str(&s)
                            .or_else(|e| Err(D::Error::custom(e.to_string())))?,
                    });
                }
            }
            ...
            Bson::Int64(timestamp) => {
                return Ok(DateTimeNative {
                    inner: chrono::NaiveDateTime::from_timestamp(
                        timestamp / 1000,
                        (timestamp % 1000 * 1000000) as u32,
                    ),
                });
            }
            v => Err(D::Error::custom(format!(
                "deserialize un supported bson value={:?},type= DateTimeNative",
                v
            ))),
        }
    }
