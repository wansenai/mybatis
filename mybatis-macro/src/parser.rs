...
fn include_replace(
    htmls: Vec<Element>,
    sql_map: &mut HashMap<String, Vec<Element>>,
) -> Vec<Element> {
    let mut results = vec![];
    for mut x in htmls {
        match x.tag.as_str() {
            "sql" => {
                sql_map.insert(
                    x.attrs
                        .get("id")
                        .expect("[mybatis] <sql> element must have id!")
                        .clone(),
                    x.childs.clone(),
                );
            }
            "include" => {
                let refid = x
                    .attrs
                    .get("refid")
                    .expect("[mybatis] <include> element must have refid!")
                    .clone();
                let data_url = Url::parse(format!("url://{}", refid).as_str())
                    .expect("[mybatis] parse include url fail!");
                    ...
                    match k.to_string().as_str() {
                        "file" => {
                            ...
                            let find = find_element(refid.as_str(), &datas).expect(&format!(
                                "[mybatis] not find html:{} , element id={}",
                                v, refid
                            ));
                            x.childs.push(find);
                            find_file = true;
                ...
                if !find_file {
                    ...
                    let element = sql_map
                        ...
                        .expect(&format!(
                            "[mybatis] can not find element {} <include refid='{}'> !",
                            refid, refid_path
                        ))
                        .clone();
                    for el in element {
                ...
                for x in &x.childs {
                    ...
                    if x.tag.eq("when") {
                        ...
                        impl_if(
                            ...
                            methods,
                            quote! {return sql;},
                            &format!("{}:{}", block_name, "choose:when:if"),
                            ignore,
                        );
                    }
                    if x.tag.eq("otherwise") {
                        ...
                        impl_otherwise(
                            ...
                            &mut inner_body,
                            methods,
                            &format!("{}:{}", block_name, "choose:otherwise"),
                            ignore,
                        );
                    }
                }
...
pub fn impl_fn_html(m: &ItemFn, args: &AttributeArgs) -> TokenStream {
    ...
    let mut f =
        File::open(file_name.as_str()).expect(&format!("File:\"{}\" does not exist", file_name));
